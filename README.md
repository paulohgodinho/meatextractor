...
```typescript
import { meatExtractor } from 'meatextractor';
...
```bash
# Server mode
docker run -p 8676:8676 ghcr.io/paulohgodinho/meatextractor:main serve

# File mode (requires mounted volume)
docker run -v $(pwd):/data ghcr.io/paulohgodinho/meatextractor:main /data/article.html
```
